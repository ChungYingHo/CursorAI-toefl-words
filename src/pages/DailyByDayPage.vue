...
const yearMonthAllOptions = computed<YearMonthOption[]>(() => {
  ...
  dailyData.value.forEach(d => ymSet.add(d.date.slice(0, 7)))
...
const filteredDays = computed(() => {
  if (selectedYearMonth.value === 'all') return dailyData.value
  return dailyData.value.filter(d => d.date.startsWith(selectedYearMonth.value + '/'))
})
